 ...
 * 
 */
public class ContactDetailActivityTest extends
		ActivityUnitTestCase<ContactDetailActivity> {
	...
	public final void testLayout() {
		...
		int[] ids = { ch.ffhs.esa.lifeguard.R.id.contactDetailNameLabel,
	...
	public final void testContact() {
		EditText nameField = (EditText) activity
				.findViewById(ch.ffhs.esa.lifeguard.R.id.contactDetailName);
		assertNotNull(nameField);
		assertEquals("Jane Doe", nameField.getText().toString());

		EditText phoneField = (EditText) activity
				.findViewById(ch.ffhs.esa.lifeguard.R.id.contactDetailPhone);
		assertNotNull(phoneField);
		assertEquals("0123456789", phoneField.getText().toString());

		Spinner positionField = (Spinner) activity
				.findViewById(ch.ffhs.esa.lifeguard.R.id.contactDetailPosition);
	...
	public final void testCancelButton() {
		Button button = (Button) activity
				.findViewById(ch.ffhs.esa.lifeguard.R.id.contactDetailCancelButton);
	...
	public final void testFailedSaveAttempt() {
		...
		Button button = (Button) activity
				.findViewById(ch.ffhs.esa.lifeguard.R.id.contactDetailSaveButton);
		assertNotNull(button);
	...
	public final void testSaveContact() {
		SQLiteDatabase database = Lifeguard.getDatabaseHelper()
				.getWritableDatabase();
	...
	private void cleanDatabase() {
		...
		this.db.getWritableDatabase().execSQL(
				"DELETE FROM sqlite_sequence WHERE NAME='contacts'");
		this.db.getWritableDatabase()
				.execSQL(
						"INSERT INTO contacts (name, phone, position) VALUES ('Jane Doe', '0123456789', 1);");
		this.db.getWritableDatabase()
				.execSQL(
						"INSERT INTO contacts (name, phone, position) VALUES ('John Doe', '0234567890', 2);");
		this.db.close();
	}
}